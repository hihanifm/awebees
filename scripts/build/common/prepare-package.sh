...
PACKAGE_SCRIPTS_DIR="$PROJECT_ROOT/scripts/windows/package"
cp "$PACKAGE_SCRIPTS_DIR/lens-start.bat" "$PACKAGE_DIR/" || {
    echo "  ✗ Error: Failed to copy lens-start.bat from $PACKAGE_SCRIPTS_DIR"
    exit 1
}
cp "$PACKAGE_SCRIPTS_DIR/lens-stop.bat" "$PACKAGE_DIR/" || {
    echo "  ✗ Error: Failed to copy lens-stop.bat from $PACKAGE_SCRIPTS_DIR"
    exit 1
}
cp "$PACKAGE_SCRIPTS_DIR/lens-status.bat" "$PACKAGE_DIR/" || {
    echo "  ✗ Error: Failed to copy lens-status.bat from $PACKAGE_SCRIPTS_DIR"
    exit 1
}
cp "$PACKAGE_SCRIPTS_DIR/lens-logs.bat" "$PACKAGE_DIR/" || {
    echo "  ✗ Error: Failed to copy lens-logs.bat from $PACKAGE_SCRIPTS_DIR"
    exit 1
}
cp "$PACKAGE_SCRIPTS_DIR/lens_tray.py" "$PACKAGE_DIR/" || {
    echo "  ✗ Error: Failed to copy lens_tray.py from $PACKAGE_SCRIPTS_DIR"
    exit 1
}
cp "$PACKAGE_SCRIPTS_DIR/lens_icon.png" "$PACKAGE_DIR/" || {
    echo "  ✗ Error: Failed to copy lens_icon.png from $PACKAGE_SCRIPTS_DIR"
    exit 1
}
cp "$PACKAGE_SCRIPTS_DIR/lens_icon_32.png" "$PACKAGE_DIR/" || {
    echo "  ✗ Error: Failed to copy lens_icon_32.png from $PACKAGE_SCRIPTS_DIR"
    exit 1
}
...
# Read build config
CONFIG_FILE="$PROJECT_ROOT/scripts/windows/build-config.json"
